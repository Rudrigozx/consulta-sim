...
Given('eu vejo um paciente com cpf {int}') do |cpf|
  fill_in 'paciente[nome]', with: 'Rodrigo'
  fill_in 'paciente[cpf]', with: cpf
  fill_in 'paciente[email]', with: 'rodrigo@gmail.com'
  fill_in 'paciente[data_de_nascimento]', with: '2000-05-06'
  fill_in 'paciente[endereco_attributes][cep]', with: 55298165
  ...
  fill_in 'paciente[endereco_attributes][bairro]', with: 'Heliopes'
  fill_in 'paciente[endereco_attributes][complemento]', with: 'proximo ao posto'
...
Then('eu vejo uma mensagem que o paciente com cpf {int} foi removido') do |cpf|
  expect(page).to have_content('Paciente was successfully destroyed.')
  expect(page).to have_no_content(cpf)
end

When('eu preecho o nome {string}, cpf {string},email {string}, data_de_nascimento {string}, cep {string}, logradouro {string},cidade {string},bairro {string} e complemento {string}') do |nome, cpf, email, data_de_nascimento, cep, logradouro, cidade, bairro, complemento|
  fill_in 'paciente[nome]', with: nome
  fill_in 'paciente[cpf]', with: cpf
  fill_in 'paciente[email]', with: email
  fill_in 'paciente[data_de_nascimento]', with: data_de_nascimento
...
Then('Eu vejo uma mensagem de paciente invalido') do
  expect(page).to have_content('19 errors prohibited this paciente from being saved:')
end
